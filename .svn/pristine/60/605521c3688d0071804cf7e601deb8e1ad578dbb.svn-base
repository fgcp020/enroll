...
public class ApplyInfoDao {
	 ...
	 * @return
	 */
	public Integer updateApplyInfoByStudentId(ApplyInfo applyInfo) {
		Criteria example = new Criteria();
		Condition condition = example.createConditon();
		condition.andEqualTo(TApplyInfo.APPLICANT, applyInfo.getApplicant());
		return this.updateByExampleSelective(applyInfo, example);
	}
}