angular.module('myServices', []).factory('services', function ($http, $rootScope) {
    ...
    $rootScope.serverAction = function (url, param, ajaxType, needFormPostCfg) {
        ...
        if (type == "GET") {
        	...
        	if(-1 < url.indexOf("user/update")) {
                ...
                return $http.put(url, param, _postCfg).error(function (data, state) {
                    console.log("系统错误:" + url.replace("?", "") + state);
                    layer.close(loading);
    ...
    var serviceAPI = {
        ...
        _login: function (param) {
            return $rootScope.serverAction('/user/login', param, "POST");
        },
        //修改密码
        ...
        _Version_list:function(param){
            return $rootScope.serverAction(ctxPath + '/admin/deviceset/selectDevices', param, "POST");
        }
